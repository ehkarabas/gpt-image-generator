...
export function ConversationSidebar() {
  ...
  const { conversations, createConversation, filteredConversations, isCreating } = useConversations()
    ...
    <div className="flex flex-col h-full" data-testid="conversation-sidebar">
      <div className="p-4 border-b border-gray-200 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900" data-testid="sidebar-title">
            Conversations
          </h2>
          <Button
            size="sm"
      ...
      <div className="flex-1 overflow-y-auto" data-testid="conversation-list-container">
        <div className="p-2">
          {displayConversations.length === 0 ? (
            <div className="text-center py-8 text-gray-500" data-testid="empty-conversations">
              <MessageSquare className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p className="text-sm">{searchQuery ? 'No conversations found' : 'No conversations yet'}</p>
            </div>
          ) : (
            <div className="space-y-1" data-testid="conversation-list" role="list" aria-label="Conversation list">
              {displayConversations.map((conversation: SidebarConversation, index: number) => (
