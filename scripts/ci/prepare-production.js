...
function validateProductionEnvironment() {
  ...
  try {
    ...
    if (!fs.existsSync(envProdPath)) {
      ...
      console.error('   3. Ensure all production API keys and URLs are configured');
      console.error('');
      console.error('🚫 Production deployment cannot proceed without proper environment configuration.');
    ...
    if (missingVars.length > 0) {
      console.error('❌ CRITICAL ERROR: Missing or empty production environment variables:');
      missingVars.forEach(envVar => console.error(`   - ${envVar}`));
      console.error('');
      console.error('🛠️  Edit .env.production and add values for all required variables');
      console.error('🔒 Ensure production URLs and API keys are properly configured');
      console.error('');
      console.error('🚫 TDD ENFORCEMENT: Production tests cannot proceed with incomplete environment');
      process.exit(1);
    }
    ...
    if (supabaseUrlMatch) {
      ...
      if (!supabaseUrl.includes('supabase.co') && !supabaseUrl.includes('localhost')) {
        console.error('❌ CRITICAL ERROR: Invalid Supabase URL format for production');
        console.error('🛠️  Ensure NEXT_PUBLIC_SUPABASE_URL points to a valid Supabase instance');
        process.exit(1);
      }
    ...
    if (!fs.existsSync(frontendEnvProdPath)) {
      console.error('❌ CRITICAL ERROR: Failed to create frontend/.env.production');
      process.exit(1);
    }
    ...
    console.error('❌ Production environment preparation failed:', error.message);
    console.error('');
    console.error('🚫 TDD ENFORCEMENT: Production environment preparation MUST succeed before deployment');
    process.exit(1);
  }
}
