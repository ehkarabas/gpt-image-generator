...
describe('MessageList', () => {
  ...
  it('renders messages in correct order (chronological)', () => {
    ...
    expect(messageItems[0]).toHaveAttribute('data-testid', 'message-item-msg-1')
    
    // Second message should be from assistant
    expect(messageItems[1]).toHaveAttribute('data-testid', 'message-item-msg-2')
    
    // Third message should be from user
    expect(messageItems[2]).toHaveAttribute('data-testid', 'message-item-msg-3')
    
    // Fourth message should be from assistant with images
    expect(messageItems[3]).toHaveAttribute('data-testid', 'message-item-msg-4')
  })
  ...
  it('auto-scrolls to bottom on new message with autoScroll enabled', async () => {
    ...
    const { rerender } = render(<MessageList messages={mockMessages.slice(0, 2)} autoScroll={true} />)
  ...
  it('does not auto-scroll when autoScroll is disabled', async () => {
    ...
    const { rerender } = render(<MessageList messages={mockMessages.slice(0, 2)} autoScroll={false} />)
