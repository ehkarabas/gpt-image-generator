...
describe('Database CRUD Operations', () => {
  ...
  afterAll(async () => {
    ...
    try {
      ...
      if (testConversationId) {
        await serviceClient.from('conversations').delete().eq('id', testConversationId)
      }
    ...
    it('should update a message', async () => {
      ...
      const { data, error } = await serviceClient
        ...
        .update({
          ...
          metadata: { tokens: 10, model: 'gpt-4', editedAt: new Date().toISOString() }
        })
        .eq('id', testMessageId)
        .select()
        .single()
  ...
  describe('Images CRUD Operations', () => {
    ...
    it('should update an image', async () => {
      ...
      expect(error).toBeNull()
      expect(data).toBeDefined()
      expect(data.revised_prompt).toBe('A stunning sunset over snow-capped mountains')
      expect(data.thumbnail_url).toBe('https://example.com/updated-thumbnail.jpg')
    })
  })
